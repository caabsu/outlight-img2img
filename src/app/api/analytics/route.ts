...
const ANALYTICS_PASSWORD = process.env.ADMIN_PASSWORD || "";
...
export async function GET(req: Request) {
  try {
    ...
    if (pass !== ANALYTICS_PASSWORD) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    ...
    const [{ data: profiles, error: profileErr }, { data: events, error: eventsErr }] = await Promise.all([
      ...
      supabase.from("usage_events").select("profile_id,model_id"),
    ]);
    ...
    return NextResponse.json({ totalsByModel, profiles: profileStats });
  } catch (err: any) {
